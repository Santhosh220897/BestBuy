...
public class CheckOutPage extends WebUtilis {
	...
	public void checkOutPayment(String firstName, String lastName, String addDetails, String place, String stateName,
		...
		// EmailAddress form
		pageWait(50);
		//waitClickable(pickupLink, 30);
		//nrmlClick(pickupLink);
		pageWait(50);
		...
		// waitClickable(continuePayment, 40);
		// jClick(continuePayment);
		...
		// Payment details facing error not able automate it.
		/*
		 * waitClickable(cardNo, 40); textKey(cardNo, cardNumber);
		 * waitClickable(expMonth, 40); dropDown(expMonth, month);
		 * waitClickable(expYear, 40); dropDown(expYear, year); waitClickable(cvv, 40);
		 * textKey(cvv, cvvNo);
		 * 
		 * Address form details scroll(); waitClickable(placeOrder, 40);
		 * jClick(placeOrder);
		 * 
		 * screenshot("PaymentPage");
		 * 
		 * String actual = elementText(orderText);
		 * 
		 * Assert.assertEquals(actual, "Place Your Order");
		 */
	}
}
